...
DAILY_DIR = Daily
EXCLUDE_DIRS = .obsidian, templates, node_modules, Daydreams, .opencode, .git
LOOKBACK_DAYS = 30
PAIR_COUNT = 50
BATCH_SIZE = 5
SCORE_THRESHOLD = 7.0
...
date +"%Y%m%d"
```
Store as `TODAY` (e.g. `20260803`).

### 1b. Find recent notes
Use Bash to list all .md files modified in the last 30 days, excluding certain directories:
...
Read `.agents/skills/daydream/history.json` if it exists. This contains previously sampled pairs:
```json
{
  "sampled_pairs": [
...
For each note found in Step 1 (up to 20 notes — if more, take the 20 most recent):
...
**Important**: Do NOT read all 20 files sequentially. Read in parallel batches of 2 using the Read tool (call 2 Read tools in a single response).
...
Read the file `.agents/skills/daydream/synthesizer-prompt.md` to get the prompt template.

### 4b. Batch pairs
Split the pairs into batches of 5. This yields up to 10 batches.

### 4c. Launch Task agents in parallel
For each batch, launch a Task agent with `model: opencode/deepseek-v4-flash-free`:
...
Read the file `.agents/skills/daydream/critic-prompt.md` to get the prompt template.
...
For each batch, launch a Task agent with `model: opencode/north-mini-code-free`:
...
Read the existing `.agents/skills/daydream/history.json` (or start fresh if missing).
...
Write back to `.agents/skills/daydream/history.json`.
...
- **No notes found**: Tell user "No notes modified in last 30 days. Nothing to daydream about."
...
- Reading 20 files: batch into groups of 2 parallel Read calls
