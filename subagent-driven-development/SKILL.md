...
description: Execute plan by dispatching a fresh implementer subagent per task, a task review (spec compliance + code quality) after each and a broad whole-branch review at the end. Use when executing implementation plans with independent tasks in the current session.
---
